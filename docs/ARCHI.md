...
- **`step_loader`** (B1.1 + B3.0 + B3.1): Prefers `STEPCAFControl_Reader` (XCAF — provides `name`, `part_name`, `color_rgb`, `material_hint` on `LeafSolid` and `name` on `AssemblyNode`). Creates the XCAF document via `TDocStd_Document("pythonocc-doc-step-import")` (pythonocc's own pattern); falls back to `TDocStd_Application` + `BinXCAFDrivers`. Name extraction uses `TDF_Label.GetLabelName()`. For assembly components that are references, the instance name (from the component label, e.g. "R41") becomes `LeafSolid.name`; the prototype name (from the referred shape label, e.g. "R_0402_1005Metric") becomes `LeafSolid.part_name`. Assembly placement transforms are accumulated via `TopLoc_Location` composition through the reference chain and baked into each leaf shape via `BRepBuilderAPI_Transform` (same pattern as pythonocc's `read_step_file_with_names_colors`). Falls back to `STEPControl_Reader` when XCAF is unavailable. The fallback produces the same `solid_NNNN` IDs and bboxes but no XCAF metadata.
...
- **`rules`** (B2.3 + B3.1): `SolidRef(solid_id, path_key, display_name, part_name)` is the stable input token. `NamingRule` maps a regex pattern to a `group_id` at a priority level. `apply_rules()` evaluates rules against all four `SolidRef` fields; highest-priority unique match wins, ties at the same priority and different `group_id` mark the solid ambiguous. Ships `default_rules.yaml` with 16 patterns.
